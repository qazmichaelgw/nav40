...
struct APM_Datatype
{
    ...
    float velocity_air; //m/s, (invalid in this model)
...
using namespace std;
// add for debug to print the info of the struct
ostream& operator<<(ostream& os, const APM_Datatype& apm) {
    return os << "State: " << static_cast<unsigned>(apm.state) << endl
              << "Roll: " << apm.roll << " Pitch: " << apm.pitch << " Yaw: " << apm.yaw << " Yaw_GPS: " << apm.yaw_gps << endl
              << "Roll Rate: " << apm.roll_rate << " Pitch Rate: " << apm.pitch_rate << " Yaw Rate: " << apm.yaw_rate << endl
              << "lon: " << apm.lon << " lat: " << apm.lat << endl
              << "alt_baro: " << apm.alt_baro << " alt_GPS: " << apm.alt_gps << " alt: " << apm.alt << endl
              << "Vel: (" << apm.velocity_x << "," <<  apm.velocity_y << "," <<  apm.velocity_z << "," << apm.velocity_air << ")" << endl
              << "ACC: (" << apm.accel_x << "," << apm.accel_y << "," << apm.accel_z << ")" << endl 
              << "satelite num: " << static_cast<unsigned>(apm.satellite_num) << endl
              << "(h,v) dop: (" << apm.hdop << "," << apm.vdop << ")" << endl
              << "GPS status: " << static_cast<unsigned>(apm.gps_status) << endl
              << "temperature: " << static_cast<unsigned>(apm.temperature) << endl
              << "redundancy: " << static_cast<unsigned>(apm.redundancy) << endl
              << "GPS time(hhmmss): " << static_cast<unsigned>(apm.gps_hh) << "， " << static_cast<unsigned>(apm.gps_mm) << ", " << static_cast<unsigned>(apm.gps_ss) << endl
              << "GPS vel: (" << apm.GPS_vx << "," << apm.GPS_vy << "," << apm.GPS_vz << ")" << endl
              << "HDT "<< apm.HDT << ", " << apm.HDG_Dev << endl
              << "##################################################################################" << endl;
}